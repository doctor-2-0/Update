...
const FindDoctor: React.FC<FindDoctorProps> = ({
  ...
  const { name, speciality, available, nearMe, perimeter } = useSelector(
    (state: RootState) => state.findDoctor
  );
  ...
  const handleSearch = () => {
    ...
    if (!selectedLocation && lastTypedLocation && lastSelectedCoords) {
      ...
      if (
        ...
        currentLocationInput.value.trim().toLowerCase() ===
          lastTypedLocation.trim().toLowerCase()
      ) {
  ...
  return (
    <>
      <Paper
        elevation={3}
        sx={{ mt: -5, borderRadius: 4, overflow: "hidden", mb: 4 }}
      >
        <Box p={2}>
              ...
              <>
                <Grid item xs={12} sm={6} md={4}>
                  <LocationSearch
                    ...
                    initialValue={lastTypedLocation}
                  />
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                  <TextField
                    ...
                    onBlur={() =>
                      dispatch(setPerimeter(parseInt(localPerimeter) || 15))
                    }
                  />
                </Grid>
              </>
